...
import { Building, Plus, CheckCircle, AlertTriangle } from 'lucide-react';
...
export const DepartmentsPage: React.FC = () => {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['departments', page, pageSize, search, includeDeleted],
    queryFn: async () => {
  ...
  const createMutation = useMutation({
    mutationFn: async (payload: any) => (await api.post('/departments', payload)).data,
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['departments'] });
      setIsFormOpen(false);
      showToast('Department created successfully!');
    },
    onError: (err: any) => showToast(err.response?.data?.detail || 'Error creating department', 'error'),
  });

  const updateMutation = useMutation({
    mutationFn: async ({ id, payload }: { id: string; payload: any }) => (await api.put(`/departments/${id}`, payload)).data,
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['departments'] });
      setIsFormOpen(false);
    ...
    onError: (err: any) => showToast(err.response?.data?.detail || 'Error updating department', 'error'),
  });
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['departments'] });
      showToast('Department soft deleted!');
    },
  });

  const restoreMutation = useMutation({
    mutationFn: async (id: string) => (await api.post(`/departments/${id}/restore`)).data,
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['departments'] });
  ...
  const columns: ColumnDef[] = [
    { key: 'code', label: 'Dept Code', render: (item) => <span className="font-mono font-bold text-blue-600">{item.code}</span> },
  ...
  const formFields: FieldFieldDef[] = [
    ...
    { name: 'code', label: 'Department Code (e.g. ENG, HR, QA)', required: true },
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-xl font-bold text-slate-900 flex items-center gap-2">
            <Building className="w-6 h-6 text-blue-600" /> Departments Master
          </h1>
          <p className="text-xs text-slate-500 font-medium mt-1">Manage corporate organizational departments.</p>
        </div>
