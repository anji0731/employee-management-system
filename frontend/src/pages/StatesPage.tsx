...
import { Map, Plus, CheckCircle, AlertTriangle } from 'lucide-react';
...
import type { Country } from '../types/employee';
...
export const StatesPage: React.FC = () => {
  ...
  const { data: countries = [] } = useQuery<Country[]>({
    ...
    queryFn: async () => (await api.get('/countries')).data?.items || (await api.get('/countries')).data || [],
  });

  const { data, isLoading } = useQuery({
    queryKey: ['states', page, pageSize, search, includeDeleted],
    queryFn: async () => {
  ...
  const createMutation = useMutation({
    mutationFn: async (payload: any) => (await api.post('/states', payload)).data,
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['states'] });
      setIsFormOpen(false);
      showToast('State record added successfully!');
    },
    onError: (err: any) => showToast(err.response?.data?.detail || 'Error creating state', 'error'),
  });

  const updateMutation = useMutation({
    mutationFn: async ({ id, payload }: { id: string; payload: any }) => (await api.put(`/states/${id}`, payload)).data,
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['states'] });
      setIsFormOpen(false);
    ...
    onError: (err: any) => showToast(err.response?.data?.detail || 'Error updating state', 'error'),
  });

  const deleteMutation = useMutation({
    mutationFn: async (id: string) => (await api.delete(`/states/${id}`)).data,
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['states'] });
      showToast('State soft deleted!');
    },
  });

  const restoreMutation = useMutation({
    mutationFn: async (id: string) => (await api.post(`/states/${id}/restore`)).data,
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['states'] });
  ...
  const columns: ColumnDef[] = [
    ...
    { key: 'code', label: 'State Code', render: (item) => <span className="font-mono text-xs text-blue-600 font-semibold">{item.code || '-'}</span> },
  ];

  const formFields: FieldFieldDef[] = [
    { name: 'name', label: 'State Name', required: true },
    { name: 'code', label: 'State Code (e.g. CA, NY, KA)' },
    {
      name: 'country_id',
      label: 'Country',
      type: 'select',
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-xl font-bold text-slate-900 flex items-center gap-2">
            <Map className="w-6 h-6 text-blue-600" /> States / Provinces Master
          </h1>
          <p className="text-xs text-slate-500 font-medium mt-1">Manage regional state definitions per country.</p>
        </div>
